...
sw_template = {'access':access_template, 'trunk':trunk_template}
vlan_q = {'access':"Enter VLAN number:", 'trunk':"Enter allowed VLANs:"}

ifmode=input("Enter interface mode (access/trunk):")
ifnumber=input("Enter interface type and number:")
vlans=input(vlan_q[ifmode])
print("interface ", ifnumber)
print("\n".join(sw_template[ifmode]).format(vlans))
