...
trunk_command = {'add': 'add', 'only': '', 'del': 'remove'}

for intf, vlan in fast_int['trunk'].items():
    print('interface FastEthernet' + intf)
    for command in trunk_template:
        if command.endswith('allowed vlan'):
            if  vlan[0] == 'add' or vlan[0] == 'remove':
                for v in vlan[1:]:
                    print(' {} {} {}'.format(command, trunk_command[vlan[0]], v))
            else:
                print(' {} {} {}'.format(command, trunk_command[vlan[0]], ",".join(vlan[1:])))
        else:
            print(' {}'.format(command))
